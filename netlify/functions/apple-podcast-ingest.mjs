...
export async function handler(event) {
  ...
  if (!supabase) {
    return response(500, { error: 'Apple Podcast ingestion is not configured' });
  }
...
export async function runApplePodcastIngest() {
  ...
  if (dueFeedsResult.error) {
    ...
    return { ok: false, error: 'Unable to load Apple Podcast feeds' };
  }
...
async function ingestFeed(feed, rulesByPartnerId) {
  ...
  try {
    ...
    const episodesWithAppleMetadata = await enrichEpisodesWithApplePodcastMetadata(
      feed,
      importableEpisodes,
    );
...
function parsePodcastRss(xml, feed) {
  ...
  return splitXmlElements(xml, 'item').flatMap((itemXml) => {
    ...
    const canonicalUrl = textContent(itemXml, 'link') ?? feed.show_url;

    return [
      {
        guid,
        appleEpisodeId: null,
        title: normalizePodcastTitle(
          textContent(itemXml, 'title') ?? 'Untitled Apple Podcast episode',
        ),
        description:
...
async function enrichEpisodesWithApplePodcastMetadata(feed, episodes) {
  const appleEpisodesByGuid = await fetchApplePodcastEpisodes(feed.apple_podcast_id);

  return episodes.map((episode) => {
    const appleEpisode = appleEpisodesByGuid.get(normalizeEpisodeGuid(episode.guid));
    if (!appleEpisode) {
      return episode;
    }
    ...
    return {
      ...episode,
      appleEpisodeId: appleEpisode.appleEpisodeId,
      canonicalUrl: appleEpisode.canonicalUrl ?? episode.canonicalUrl,
      imageUrl: episode.imageUrl ?? appleEpisode.imageUrl,
    };
  });
}

async function fetchApplePodcastEpisodes(applePodcastId) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);

  try {
    const response = await fetch(
      `https://itunes.apple.com/lookup?id=${encodeURIComponent(applePodcastId)}&entity=podcastEpisode&limit=200`,
      {
        headers: {
          'user-agent': USER_AGENT,
    ...
    return new Map(
      (data.results ?? [])
        .filter((item) => item.wrapperType === 'podcastEpisode' && item.episodeGuid)
        .map((item) => [
          normalizeEpisodeGuid(item.episodeGuid),
          {
            appleEpisodeId: String(item.trackId),
            canonicalUrl: stripTrackingParameter(item.trackViewUrl),
            imageUrl: item.artworkUrl600 ?? item.artworkUrl160 ?? null,
          },
        ]),
    );
  } catch {
...
function normalizeEpisodeGuid(value) {
  return String(value ?? '')
    .replace(/&amp;/g, '&')
    .replace(/&#0*38;/g, '&')
    .trim();
...
function normalizeEpisode(feed, parsedEpisode, rules, existingClassification) {
  ...
  return {
    ...
    feed_id: feed.id,
    apple_episode_id: appleEpisodeId,
    ...
    embed_url: appleEpisodeId
      ? `${feed.embed_url}?i=${appleEpisodeId}`
      : feed.embed_url,
...
function stripTrackingParameter(value) {
  if (!value) {
    return null;
  }
...
function normalizePodcastTitle(value) {
...
function logInfo(message, detail = {}) {
  console.info(`[apple-podcast-ingest] ${message}`, JSON.stringify(detail));
}

function logError(message, detail = {}) {
  console.error(`[apple-podcast-ingest] ${message}`, JSON.stringify(detail));
}
